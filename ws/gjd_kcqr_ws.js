...
define(['N/search', 'N/record', 'N/log', 'N/ui/dialog', 'N/file'], (search, record, log, file) => {
	...
	const onAction = (context) => {
		try {
			...
			//加载估价单
			var gjdRecord = record.load({ type: 'estimate', id: recId });
			if (gjdRecord) {
				//后续是扣减当前业务员库存逻辑
				var itemType = 'item';
				...
				for (var i = 0; i < hpItemCount; i++) {
					...
					var ywy_kc_id = gjdRecord.getSublistValue({ sublistId: itemType, fieldId: 'custcol18', line: i }); //业务员库存分配表

					if (!isEmpty(ywy_kc_id)) {
						var xq_hp_sl = gjdRecord.getSublistValue({ sublistId: itemType, fieldId: 'quantity', line: i }); //当前货品数量
						//可以扣减为负数
						var qr_hp = {}; //确认扣减的货品
						...
						var kc_none_hp = gjdRecord.getSublistValue({ sublistId: itemType, fieldId: 'item_display', line: i });
				...
				try {
					gjdRecord.setValue({ fieldId: 'custbody_inventory_confirm_status', value: 3 }); //库存确认状态 = 3（已确认）
					gjdRecord.setValue({ fieldId: 'custbody_txn_approve_status', value: 22 }); //客户确认PI
					gjdRecord.save();//保存记录
				} catch (e) {
					log.error('保存失败', e);
				}
