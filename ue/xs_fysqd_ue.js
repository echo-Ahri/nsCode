...
define(['N/file', 'N/log', 'N/runtime', 'N/url', 'N/record', 'N/search'], (file, log, runtime, url, record, search) => {
    const beforeLoad = (context) => {
        ...
        var fileObj = file.load({
            id: 'SuiteScripts/dsp_scripts/cs/xs_fysqd_cs.js',
        });
        form.clientScriptFileId = fileObj.id;

        var dj_sp_status = rec.getValue('custrecord_fysqd_sp_status'); //审批状态

        if(dj_sp_status == 12){
            form.addButton({
                id: 'custpage_jump_srzd_btn',
                label: '[输入账单]',
                functionName: 'jumpSrzd(' + rec_id + ',"' + rec_type + '")',
            });
        }
    };
