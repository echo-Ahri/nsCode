...
define(['N/search', 'N/record', 'N/log', 'N/ui/dialog', 'N/file'], (search, record, log, file) => {
	...
	const onAction = (context) => {
		try {
			...
			log.debug('工作流触发', '主记录ID: ' + recId);

			//加载商机
			var sjRecord = record.load({
				type: 'opportunity',
				id: recId,
			});
			if (sjRecord) {
				// dialog.confirm({
				// 	title: '取消发起库存确认',
				// 	message: '取消发起库存确认提示, 点击[OK], 将取消发起库存确认, 还原相应占用计划库存, 解锁单据.',
				...
				if (qr_kc_none_arr.length > 0) {
					// dialog.alert({ title: '错误提示', message: '当前有货品行未选择业务员库存表, 还原库存失败', });
				...
				try {
					//设置 库存确认状态 = 1(待发起)
					sjRecord.setValue({
						fieldId: 'custbody_inventory_confirm_status',
						value: 1,
						ignoreFieldChange: true,
					});
					sjRecord.setValue({
						fieldId: 'custbody_txn_approve_status', //审批状态=1 待审批
						value: 1,
						ignoreFieldChange: true,
					});
					//保存记录
					sjRecord.save();
					// log.debug('操作成功', '商机保存成功。');
				} catch (e) {
					log.error('保存失败', e);
				}
				// });
