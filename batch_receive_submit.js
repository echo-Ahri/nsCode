...
define(['N/ui/serverWidget', 'N/search', 'N/redirect', 'N/runtime', 'N/ui/message', 'N/record', 'N/url', 'N/log', 'N/file'],
    function (serverWidget, search, redirect, runtime, message, record, url, log, file,) {
        ...
        function createForm(context) {
            var form = serverWidget.createForm({ title: '批量接收结算单并创建核价单' }); //批量发送核算单  业务 -> 协同   协同 -> 执行   执行 -> 财务(创建核算单)
        ...
        function getPricingOrderData(context) {
            ...
            var pricingOrderSearch = search.load({
                id: 'customsearch_jsd_batch_use_jb_js'  // 结算单接收数据（批量接收脚本用，勿动）
            });
            ...
            pricingOrderSearch.run().each(function (result) {
                pricingOrder.push({
                    ...
                    custpage_name: result.getValue('name'),//核价单编号
                ...
                if (isChecked === 'T') {
                    ...
                    if (recordId) {
                        try {
                            ...
                            // 主记录字段映射表
                            /* var fieldMapping = {
                                custrecord_pricingmain_subsidiary: 'custrecord_accounting_company',
                                custrecord_pricing_main_person: 'custrecord_accounting_salesman',
                                custrecord_pricingmain_customer: 'custrecord_accounting_final_customer',
                                custrecord_pricingmain_document_type: 'custrecord_accounting_document_type',
                                custrecord_pricingmain_currency: 'custrecord_accounting_currency',
                                custrecord_pricing_main_allpayment_date: 'custrecord_accounting_allpayment_date',
                                custrecord_pricingmain_service_route: 'custrecord_accounting_service_route',
                                custrecord_pricingmain_hl: 'custrecord_accounting_hl',
                                custrecord_pricing_main_collection_date: 'custrecord_accounting_collection_date',
                                custrecord_pricingmain_business_phase: 'custrecord_accounting_business_phase',
                                custrecord_pricing_main_shipment_date: 'custrecord_accounting_shipment_date',
                                custrecord_pricing_rel_po: 'custrecord_accounting_rel_po',
                                custrecord_pricingmain_total_freight: 'custrecord_accounting_total_freight'
                            }; */
                            var fieldMapping = {
                                custrecord_accounting_company: 'custrecord_pricingmain_subsidiary',
                                custrecord_accounting_salesman: 'custrecord_pricing_main_person',
                                custrecord_accounting_final_customer: 'custrecord_pricingmain_customer',
                                custrecord_accounting_document_type: 'custrecord_pricingmain_document_type',
                                custrecord_accounting_currency: 'custrecord_pricingmain_currency',
                                custrecord_accounting_allpayment_date: 'custrecord_pricing_main_allpayment_date',
                                custrecord_accounting_service_route: 'custrecord_pricingmain_service_route',
                                custrecord_accounting_hl: 'custrecord_pricingmain_hl',
                                custrecord_accounting_collection_date: 'custrecord_pricing_main_collection_date',
                                // custrecord_accounting_business_phase: 'custrecord_pricingmain_business_phase', //下面动态更新
                                // custrecord_accounting_next_service: 'custrecord_pricingmain_next_service', //下一贸易流转
                                custrecord_accounting_shipment_date: 'custrecord_pricing_main_shipment_date',
                                custrecord_accounting_rel_po: 'custrecord_pricing_rel_po',
                                custrecord_accounting_total_freight: 'custrecord_pricingmain_total_freight',
                                custrecord_accounting_tradetype: 'custrecord_pricingmain_tradetype', //销售类型
                                custrecord_accounting_payment_method: 'custrecord41', //付款方式
                                custrecord_accouting_place_of_delivery: 'custrecord_place_of_delivery', //货品交付地
                                custrecord_accounting_trade_destination: 'custrecord_trade_destination' //货品目的地
                            };
                            ...
                            var sublistMapping = [
                                {
                                    ...
                                    targetSublistId: 'recmachcustrecord_freightline_pricingmain' // 新记录目标子列表ID
                                },
                                {
                                    sourceSublistId: 'recmachcustrecord_misline_accountmain', // 第二个子列表
                                    targetSublistId: 'recmachcustrecord_misline_pricingmain' // 对应目标子列表
                                },
                                {
                                    sourceSublistId: 'recmachcustrecord_pricing_woan', // 第三个子列表
                                    targetSublistId: 'recmachcustrecord_pricing_wopmn' // 对应目标子列表
                                }
                            ];
                            ...
                            updatedAccountingRecord.setValue({
                                fieldId: 'name', // 核价单的名称字段
                                value: sourceName // 更新为源记录的名称值
                            });

                            updatedAccountingRecord.setValue({
                                fieldId: 'custrecord_pricingmain_create_from', // 核价单 创建自链接 的名称字段
                                value: poUrl // 更新为记录的创建自
                            });

                            // log.debug("当前阶段", parseInt(sourceRecord.getValue({ fieldId: 'custrecord_accounting_business_phase' })) + 1);
                            // throw "退出 测试当前阶段";
                            //业务1 协同2 执行3 财务4
                            updatedAccountingRecord.setValue({
                                fieldId: 'custrecord_pricingmain_business_phase', // 核价单 处理阶段 的名称字段
                                value: parseInt(sourceRecord.getValue({ fieldId: 'custrecord_accounting_business_phase' })) + 1 // 更新为下一阶段
                            });
                            updatedAccountingRecord.setValue({
                                fieldId: 'custrecord_pricingmain_next_service', // 核价单 下一贸易流转 的名称字段
                            ...
                            log.error('创建核价单失败', `来源记录ID: ${recordId}, 错误信息: ${e.message}`);
                        }
                    }
                }
            }

            log.debug('创建的核价单记录ID', createdIds.join(', '));

            // 返回用户操作反馈
            if (createdIds.length > 0) {
                log.audit('核价单创建成功', `以下核价单已创建: ${createdIds.join(', ')}`);
                context.response.write('<p>成功创建以下核算单: ' + createdIds.join(', ') + '</p>');
            } else {
                log.audit('无核价单创建', '没有选中记录进行创建操作');
